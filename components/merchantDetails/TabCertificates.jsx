    ...
    <>
        ...
        <div className='mt-4 gap-x-8 w-full'>
            <div className='w-full'>
                <h1 className='font-bold text-[22px]'>Certificates </h1>
            </div>
            <div className='mt-5 flex flex-wrap gap-x-6 gap-y-5'>
                
                {
                    imgList.map((src, index)=>{
                        return(
                            <div key={index} className='flex justify-center items-center bg-[#F6F6F6] border drop-shadow-md w-64 h-44 rounded-md cursor-pointer' onClick={()=>setImgOpen(src)}>
