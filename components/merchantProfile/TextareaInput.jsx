...
const TextareaInput = ({id, required, width, height, title, placeholder, warning, onChange, value, error, wordLimit, readOnly}) => {
  ...
  return (
    <div className={`flex flex-wrap ${width} relative`}>
        ...
        <textarea value={value?value:''} className={`py-3 px-4 ${title ? 'mt-2':'mt-0'} w-full ${height} font-normal text-base ${readOnly ? 'bg-[#F9F9F9]' : 'bg-white'} border ${error === false || error === undefined ? 'border-[#E9E9E9]' : 'border-red-500'} rounded-md`} placeholder={placeholder} type="text" id={id} required={required} onChange={e=>handleChange(e)} readOnly={readOnly}/>
